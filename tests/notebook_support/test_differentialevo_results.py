...
from notebook_support.differentialevo_results import (
    load_differentialevo_results,
    plot_differentialevo_double_cv_parity,
    resolve_latest_differentialevo_run,
)


def test_resolve_latest_differentialevo_run_picks_expected_timestamped_bundle(tmp_path) -> None:
    run_root = _create_differentialevo_fixture(tmp_path)
    older = run_root.parent / "20260313T101647Z"
    older.mkdir(parents=True)

    resolved = resolve_latest_differentialevo_run(results_root=tmp_path / "results")
    assert resolved == run_root
    assert resolved.parent.name == "differentialevo"


def test_load_differentialevo_results_aggregates_all_folds_and_tables(tmp_path) -> None:
    run_dir = _create_differentialevo_fixture(tmp_path)
    payload = load_differentialevo_results(run_dir)
    ...
    assert list(parameter_summary.columns) == ["parameter", "mean_value", "std_value", "low", "high", "scale"]
    assert len(parameter_summary) == 8

    metrics_summary = payload["metrics_summary_table"]
    assert set(metrics_summary["split"]) == {"train", "test"}
    assert payload["population_summary_table"].empty is False
...
def test_plot_differentialevo_double_cv_parity_returns_two_axes_and_shared_figure(tmp_path) -> None:
    payload = load_differentialevo_results(_create_differentialevo_fixture(tmp_path))
    figure = plot_differentialevo_double_cv_parity(payload)
...
def _create_differentialevo_fixture(tmp_path: Path) -> Path:
    run_dir = tmp_path / "results" / "remifentanil_benchmark" / "differentialevo" / "20260314T120000Z"
    ...
    _write_yaml(
        run_dir / "config.yaml",
        {
            "resolved_config": {
                ...
                "optimizer": {"population_size": 8, "num_generations": 4},
            }
        },
    )
    ...
    for fold_index in range(5):
        ...
        _write_yaml(
            fold_dir / "differentialevo_metadata.yaml",
            {
                "best_objective": float(10.0 + fold_index),
                "initial_best_objective": float(12.0 + fold_index),
                "n_evaluations": 40,
        ...
        best_params.to_parquet(fold_dir / "differentialevo_best_params.parquet", index=False)
            ...
            [
                {
                    ...
                    "rank": member_index,
                    "fitness": float(10.0 + member_index + fold_index),
                    "k_TP": 0.1,
                    "k_PT": 0.2,
                    "k_PHP": 0.3,
                    "k_HPP": 0.4,
        ...
        final_population.to_parquet(fold_dir / "differentialevo_final_population.parquet", index=False)
